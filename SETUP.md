...
# OPSKY NFC-to-CAN J1939 Bridge Setup Guide

This document describes how to set up and run the NFC-to-CAN J1939 bridge system using the provided Python services in the `opsky_nfc_can_ble` folder.

## Prerequisites

- **Hardware:**
  - Raspberry Pi 4B (or compatible Linux SBC)
   - PN532 NFC HAT (supports UART or I2C connection)
...
**Note:** Enable UART or I2C as needed in `raspi-config` (Interface Options).
...
2. **Install dependencies:**
   ...
   pip install -r requirements_j1939.txt
   ```
   Ensure you have the following packages (see `requirement.txt` and `requirements_j1939.txt`):
   - python-can
   - can-j1939
   - pyzmq
   - RPi.GPIO
...
- **NFC parameters** (in `nfc_service.py`):
   - UART/I2C mode selection (see comments in code)
...
3. **Start the CAN J1939 Service:**
   ...
   ./can_start.sh
   ```
   (This will set up the CAN interface and run the CAN J1939 service)
...
4. 'bleToCan' and 'canToBle' topics are handled by can_j1939_service.py for integration with BLE (if used).
...
- Use `dmesg` or `candump can0` for CAN debugging (install can-utils if needed).
...
- Edit `nfc_service.py` for NFC read interval, UART/I2C mode, or pin if needed.
